...
void test_bound_propagator_soundness(const char* pred_name, F pred, bool test_completeness = true, bool disable_before_test = false) {
  int minval = -5;
  int maxval = 5;
  std::vector<Itv> itvs{Itv(minval, minval), Itv(minval+1, maxval-1), Itv(minval,-3), Itv(minval,-2), Itv(minval,0), Itv(minval,maxval), Itv(-2,-1), Itv(-1,0), Itv(-2,2), Itv(-1,-1), Itv(0,0), Itv(1,1), Itv(2,2), Itv(0,1), Itv(1,2), Itv(0, maxval), Itv(2, maxval), Itv(3, maxval), Itv(maxval, maxval)};
